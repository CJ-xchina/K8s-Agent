...
class baseMemory(object):
    ...
    def __init__(self, interval_seconds: int, get_graph_manager_func: Callable):
        """
        初始化 MemoryBase，并在创建后立即启动自动刷新。

        Args:
            interval_seconds (int): 自动刷新的时间间隔，单位为秒。
            get_graph_manager_func (Callable): 获取最新图表信息的函数，用于 NodeMemoryItem 的更新。
        """
        ...
        self.action_map: Dict[str, NodeMemoryItem] = {}  # 快速查找的action -> NodeMemoryItem 映射表
        ...
        self.get_graph_manager_func = get_graph_manager_func
    ...
    def store_data(self, item: NodeMemoryItem):
        ...
        """
        # 如果该 action 已经存在，则更新旧的 NodeMemoryItem 而不是直接替换
        ...
        # 将 NodeMemoryItem 添加到优先队列（使用负时间戳确保最新时间在前）
    ...
    def _extract_fields(self, item: NodeMemoryItem, fields: Optional[List[str]]) -> Dict[str, Any]:
        """
        从 NodeMemoryItem 中提取指定字段，如果没有指定字段则返回完整的数据。

        Args:
            item (NodeMemoryItem): 要提取数据的 NodeMemoryItem 对象。
            fields (List[str], optional): 需要提取的字段列表。

        Returns:
            Dict[str, Any]: 包含提取字段或完整 NodeMemoryItem 的字典。
        """
        if not fields:
    ...
    def get_data(self, fields: Optional[List[str]] = None) -> list[dict[str, Any]]:
        ...
        如果未指定 fields，则返回所有 NodeMemoryItem 数据。

        Args:
            fields (List[str], optional): 要提取的 NodeMemoryItem 属性字段。如果为空，则返回完整的 NodeMemoryItem 数据。

        Returns:
            str: 包含 NodeMemoryItem 数据的 JSON 字符串。
        """
        result = []
    ...
    def get_memory_item_by_action(self, action: str) -> NodeMemoryItem:
        """
        根据 action 查找并返回对应的 NodeMemoryItem。

        Args:
            action (str): 要查找的 action 名称。

        Returns:
            Optional[NodeMemoryItem]: 返回找到的 NodeMemoryItem，如果不存在则返回 None。
        """
        return self.action_map.get(action)

    def remove_memory_item_by_action(self, action: str):
        """
        根据 action 删除对应的 NodeMemoryItem。

        Args:
            action (str): 要删除的 action 名称。
        """
        # 从优先队列和 action_map 中删除 NodeMemoryItem
        self.memory_store = [entry for entry in self.memory_store if entry[1] != action]
        if action in self.action_map:
            del self.action_map[action]
            print(f"已删除 action: {action} 对应的 NodeMemoryItem")
        else:
            print(f"未找到 action: {action} 对应的 NodeMemoryItem")

    def get_all_summaries(self) -> str:
        """
        获取所有 NodeMemoryItem 的 Action、Description、Question 以及其对应的 NodeId 和 Conclusion。
        返回格式化的字符串，按你要求的格式输出。
        """
        summary = []
