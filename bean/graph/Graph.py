...
class Graph:
    ...
    def get_node_if_statement(self, node_id: str) -> List[str]:
        """
        获取当前节点的if语句条件。
    ...
    def jump_to_node_by_condition(self, current_node_id: str, condition_value: Optional[str] = None) -> Optional[str]:
        """
        ...
        :return: 下一个节点的 ID，如果没有找到则返回 None。
        """
        current_node = self.get_node(current_node_id)
        if not current_node:
            raise ValueError(f"当前节点 ID {current_node_id} 不存在。")
            ...
            for edge in edges:
                if edge.condition_value == condition_value:
                    ...
                    if target_group_node and target_group_node.node_type == "group":
                        return self.find_group_input(current_node_id)
                    else:
                        raise Exception(
    ...
    def find_group_input(self, current_node_id):
        current_node = self.get_node(current_node_id)
        input_nodes = [
        ...
        if len(input_nodes) == 1:
            current_node_id = input_nodes[0].node_id
            print(f"跳转到 group {current_node.node_id} 中的 input 节点 {current_node_id}。")
